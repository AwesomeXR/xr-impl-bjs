...
import { BufferDataReader } from './Misc/BufferDataReader';
...
export class KTX2FileReader {
  ...
  public parse() {
     ...
     * Get the header
     */
    const hdrReader = new BufferDataReader(this._data, offsetInFile, 17 * 4);
    ...
    const levelReader = new BufferDataReader(this._data, offsetInFile, levelCount * 3 * (2 * 4));
     ...
     */
    const dfdReader = new BufferDataReader(this._data, header.dfdByteOffset, header.dfdByteLength);
    ...
    if (header.sgdByteLength > 0) {
      const sgdReader = new BufferDataReader(this._data, header.sgdByteOffset, header.sgdByteLength);
      ...
      sgd.endpointsData = new Uint8Array(
        this._data.buffer,
        this._data.byteOffset + endpointsByteOffset,
        sgd.endpointsByteLength
      );
      sgd.selectorsData = new Uint8Array(
        this._data.buffer,
        this._data.byteOffset + selectorsByteOffset,
        sgd.selectorsByteLength
      );
      sgd.tablesData = new Uint8Array(
        this._data.buffer,
        this._data.byteOffset + tablesByteOffset,
        sgd.tablesByteLength
      );
      sgd.extendedData = new Uint8Array(
        this._data.buffer,
        this._data.byteOffset + extendedByteOffset,
        sgd.extendedByteLength
      );
    }
  }
  ...
  public get textureFormat(): KTX2.SourceTextureFormat {
    return this._dfdBlock.colorModel === DFDModel.UASTC
      ? KTX2.SourceTextureFormat.UASTC4x4
      : KTX2.SourceTextureFormat.ETC1S;
  }
    ...
    switch (tformat) {
      case KTX2.SourceTextureFormat.ETC1S:
        return (
          ...
          (this._dfdBlock.samples[0].channelType === DFDChannel_ETC1S.AAA ||
            this._dfdBlock.samples[1].channelType === DFDChannel_ETC1S.AAA)
        );
