...
export class LiteTranscoder extends Transcoder {
  ...
  protected _loadModule(): Promise<{ module: any }> {
    ...
    this._modulePromise = WASMMemoryManager.LoadWASM(this._modulePath).then(wasmBinary => {
      return new Promise(resolve => {
        WebAssembly.instantiate(wasmBinary as ArrayBuffer, { env: { memory: this._memoryManager.wasmMemory } }).then(
          moduleWrapper => {
  ...
  public transcode(
    ...
    return this._loadModule().then((moduleWrapper: any) => {
      ...
      const [textureView, uncompressedTextureView, nBlocks] = this._prepareTranscoding(
        width,
        height,
      ...
      return transcoder.transcode(nBlocks) === 0
        ? this._transcodeInPlace
          ? textureView.slice()
          : uncompressedTextureView!.slice()
        : null;
    });
  }
