...
export type KTX2ExtraOption = {
  transcodeFormat?: KTX2.TranscodeTarget;
  engineFormat?: KTX2.EngineFormat;
...
export class KTX2Decoder {
  ...
  public decode(
    data: Uint8Array,
    caps: KTX2.ICompressedFormatCapabilities,
    options?: KTX2.IKTX2DecoderOptions,
    extra: KTX2ExtraOption = {}
  ): Promise<KTX2.IDecodedData> {
    ...
    return Promise.resolve().then(() => {
      ...
      if (kfr.needZSTDDecoder) {
        ...
        return this._zstdDecoder.init().then(() => {
          return this._decodeData(kfr, caps, finalOptions, extra);
        });
      }
  ...
  private _decodeData(
    ...
    const decisionTree = new TranscodeDecisionTree(
      srcTexFormat,
      kfr.hasAlpha,
    ...
    const transcodeFormat =
      typeof extra.transcodeFormat !== 'undefined' ? extra.transcodeFormat : decisionTree.transcodeFormat;

    const engineFormat = typeof extra.engineFormat !== 'undefined' ? extra.engineFormat : decisionTree.engineFormat;

    const roundToMultiple4 =
      typeof extra.roundToMultiple4 !== 'undefined' ? extra.roundToMultiple4 : decisionTree.roundToMultiple4;

    const transcoder = this._transcoderMgr.findTranscoder(
      srcTexFormat,
      transcodeFormat,
      kfr.isInGammaSpace,
      options?.bypassTranscoders
    );
    ...
    for (let level = 0; level < kfr.header.levelCount; level++) {
      if (level > 0) {
        firstImageDescIndex +=
          Math.max(kfr.header.layerCount, 1) * kfr.header.faceCount * Math.max(kfr.header.pixelDepth >> (level - 1), 1);
      }
      ...
      if (kfr.header.supercompressionScheme === SupercompressionScheme.ZStandard) {
        levelDataBuffer = this._zstdDecoder.decode(
          new Uint8Array(levelDataBuffer, levelDataOffset, kfr.levels[level].byteLength),
